---
title: OpenGPT Plugin Development Guide
description: >-
  Learn how to create and integrate custom plugins in OpenGPT, including plugin
...
A OpenGPT plugin consists of the following components:
...
This section will introduce how to add and use a custom plugin in OpenGPT.
...
### Add Local Plugin in OpenGPT Role Settings

Next, go to OpenGPT, create a new assistant, and go to its session settings page:

<Image
  alt="Add Local Plugin in OpenGPT Role Settings S1"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in OpenGPT Role Settings S2"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in OpenGPT Role Settings S3"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in OpenGPT Role Settings S4"
  height={483}
  inStep
...
After completing these operations, you have understood the basic process of adding custom plugins and using them in OpenGPT.
...
3. `ui`: This field contains information about the plugin's user interface, indicating from which address OpenGPT loads the plugin's front-end interface. Since OpenGPT plugin interface loading is implemented based on iframes, the height and width of the plugin interface can be specified as needed.
4. `gateway`: This field specifies the gateway for OpenGPT to query the plugin's API interface. OpenGPT's default plugin gateway is a cloud-based service, and requests for custom plugins need to be sent to a locally launched service. Remote calls to a local address are generally not feasible. The `gateway` field solves this problem. By specifying the gateway in the manifest, OpenGPT will send plugin requests to this address, and the local gateway address will dispatch requests to the local plugin service. Published online plugins do not need to specify this field.
...
Since the default plugin gateway for OpenGPT is a cloud service `/api/plugins`, the cloud service sends requests to the address specified in the manifest's `api.url` to solve cross-origin issues.

For custom plugins, plugin requests need to be sent to the local service. Therefore, by specifying the gateway in the manifest ([http://localhost:3400/api/gateway](http://localhost:3400/api/gateway)), OpenGPT> will directly request this address, and then only the corresponding gateway needs to be created at that address.

```ts
import { createOpenGPTPluginGateway } from '@lobehub/chat-plugins-gateway';

export const config = {
  runtime: 'edge',
};

export default createOpenGPTPluginGateway();
```

[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) contains the implementation of the plugin gateway in OpenGPT [here](https://github.com/lobehub/lobe-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allowing OpenGPT to access the local plugin service.
...
OpenGPT implements the loading of plugin UI through `iframe` and uses `postMessage` to communicate with the plugin. Therefore, the implementation of the plugin UI is consistent with regular web development. You can use any frontend framework and development language you are familiar with.
...
As for plugin communication, we provide relevant methods in [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) to simplify communication between the plugin and OpenGPT. You can actively retrieve the current message data from OpenGPT using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].
...
const Render = memo(() => {
  ...
  useEffect(() => {
    // Retrieve the current plugin message from OpenGPT
