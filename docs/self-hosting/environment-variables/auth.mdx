---
title: OpenGPT Authentication Service Environment Variables
description: >-
  Explore the essential environment variables for configuring authentication
  services in OpenGPT, including OAuth SSO, NextAuth settings, and
...
OpenGPT provides a complete authentication service capability when deployed. The following are the relevant environment variables. You can use these environment variables to easily define the identity verification services that need to be enabled in OpenGPT.
...
- Type: Required
- Description: Enable single sign-on (SSO) for OpenGPT. Set to `1` to enable single sign-on.
- Default: `-`
- Example: `1`
