---
title: Customizing Provider Model List in OpenGPT for Deployment
description: >-
  Learn how to customize the model list in OpenGPT for deployment with the
...
OpenGPT supports customizing the model list during deployment. You can use `+` to add a model, `-` to hide a model, and use `model name=display name<extension configuration>` to customize the display name of a model, separated by English commas. The basic syntax is as follows:
