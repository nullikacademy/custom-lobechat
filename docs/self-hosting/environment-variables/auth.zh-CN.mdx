---
title: OpenGPT 身份验证服务设置
description: >-
  了解如何配置 OpenGPT 的身份验证服务环境变量，包括单点登录 (SSO)、Auth0、Microsoft Entra
...
OpenGPT 在部署时提供了完善的身份验证服务能力，以下是相关的环境变量，你可以使用这些环境变量轻松定义需要在 OpenGPT 中开启的身份验证服务。
...
- 类型：必选
- 描述：为 OpenGPT 启用单点登录 (SSO)。设置为 `1` 以启用单点登录。
- 默认值: `-`
- 示例: `1`
