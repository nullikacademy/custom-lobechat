---
title: OpenGPT Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for OpenGPT Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest OpenGPT image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running OpenGPT Container

Assuming the OpenGPT container is named `lobe-chat`, use the following commands to stop and remove the currently running OpenGPT container:
...
### Pull the Latest OpenGPT Image

Use the following command to pull the latest Docker image for OpenGPT:
...
Redeploy the OpenGPT container using the newly pulled image:
...
No need to worry, you won't. All of OpenGPT's chat records are stored in your local browser. Therefore, when redeploying OpenGPT using Docker, your chat records will not be lost.
