---
title: Using Local Qwen Model in OpenGPT
description: 通过OpenGPT和Ollama的集成，您可以轻松在OpenGPT中使用Qwen。本文将指导您如何在OpenGPT中使用本地部署版本的Qwen。
tags:
  - Qwen
  - OpenGPT
  - Ollama
...
Now, through the integration of OpenGPT and [Ollama](https://ollama.com/), you can easily use Qwen in OpenGPT. This document will guide you on how to use the local deployment version of Qwen in OpenGPT:
...
In the OpenGPT conversation page, open the model selection panel, and then select the Qwen model.
...
/>
  ...
  <Callout type={'info'}>
    If you do not see the Ollama provider in the model selection panel, please refer to [Integration with Ollama](/docs/self-hosting/examples/ollama) to learn how to enable the Ollama provider in OpenGPT.

</Callout>
</Steps>

Next, you can have a conversation with the local Qwen model in OpenGPT.
